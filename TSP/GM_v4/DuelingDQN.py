...
class DuelingNetWork(nn.Module):
    def __init__(self, alpha, state_dim, action_dim, hidden_dim, hidden_layer, ope_dim, gat_hidden_dim, gat_output_dim):
        ...
        self.static_gat = OpeModel(ope_dim, gat_hidden_dim, gat_output_dim)
        self.dynamic_gat = OpeModel(ope_dim, gat_hidden_dim, gat_output_dim)
        self.state_rnn = nn.LSTM(input_size=state_dim, hidden_size=state_dim, num_layers=1,
                                 bias=True, batch_first=False, dropout=0.0, bidirectional=False)
        self.insert_gat = InsertPositionModel(state_dim, gat_hidden_dim, gat_output_dim)
    ...
    def forward(self, node_fea, static_adj, dynamic_adj, insert_adj, last_node, insert_node, h, c):
        static_gat = self.static_gat(node_fea, static_adj)
        dynamic_gat = self.dynamic_gat(node_fea, dynamic_adj)
        node_gat = torch.cat([static_gat, dynamic_gat, node_fea], dim=-1)

        state_1 = node_gat.mean(dim=-2)
        state_2 = node_gat[torch.where(last_node == 1)]
        ...
        # state_dim = node_gat.shape[-1]
        # node_num = node_gat.shape[-2]
        # blank_node_num = insert_adj.shape[-1]
        # insert_node_num = blank_node_num - 2
        # blank_node = blank_node.reshape(-1, node_num, 1).repeat(1, 1, state_dim)
        # blank_node_fea = node_gat.detach()[torch.where(blank_node == 1)].view(-1, blank_node_num, state_dim)
        # blank_gat = self.insert_gat(blank_node_fea, insert_adj)
        #
        # insert_node = insert_node.reshape(-1, node_num, 1).repeat(1, 1, state_dim)
        # insert_gat = node_gat[torch.where(insert_node == 1)].view(-1, insert_node_num, state_dim)
        #
        # blank_dim = blank_gat.shape[-1]
        # blank_num = blank_gat.shape[-2]
        # node_num = insert_gat.shape[-2]
        # action = torch.cat(
        #     [
        #         insert_gat.repeat(1, 1, blank_num).view(-1, node_num * blank_num, state_dim),
        #         blank_gat.repeat(1, node_num, 1).view(-1, node_num * blank_num, blank_dim)
        #     ],
        #     dim=-1
        # )
...
class DuelingDQN:
    ...
    def choose_action(self, state, h, c, isTrain=True):
        ...
        if (np.random.random() < self.epsilon) and isTrain:
            ...
            static_adj = T.tensor(state[1], dtype=T.int32).to(device)
            dynamic_adj = T.tensor(state[2], dtype=T.int32).to(device)
            insert_adj = T.tensor(state[3], dtype=T.float).to(device)
            blank_node = T.tensor(state[4], dtype=T.int32).to(device)
            insert_node = T.tensor(state[5], dtype=T.int32).to(device)

            h = T.tensor(h, dtype=T.float).to(device)
            c = T.tensor(c, dtype=T.float).to(device)
            _, A, hn, cn = self.q_eval.forward(node_fea, static_adj, dynamic_adj, insert_adj, blank_node, insert_node,
                                               h, c)
        else:
            node_fea = T.tensor(state[0], dtype=T.float).to(device)
            static_adj = T.tensor(state[1], dtype=T.int32).to(device)
            ...
            blank_node = T.tensor(state[4], dtype=T.int32).to(device)
            insert_node = T.tensor(state[5], dtype=T.int32).to(device)
            action_mask = T.tensor(state[6], dtype=T.int32).to(device)
    ...
    def learn(self):
        ...
        node_fea_tensor = T.tensor(np.array(node_fea), dtype=T.float).to(device)
        static_adj_tensor = T.tensor(np.array(static_adj), dtype=T.float).to(device)
        dynamic_adj_tensor = T.tensor(np.array(dynamic_adj), dtype=T.float).to(device)
        insert_adj_tensor = T.tensor(np.array(insert_adj), dtype=T.float).to(device)
        blank_node_tensor = T.tensor(np.array(blank_node), dtype=T.float).to(device)
        insert_node_tensor = T.tensor(np.array(insert_node), dtype=T.float).to(device)
        action_mask_tensor = T.tensor(np.array(action_mask), dtype=T.float).to(device)
        ...
        next_node_fea_tensor = T.tensor(np.array(next_node_fea), dtype=T.float).to(device)
        next_static_adj_tensor = T.tensor(np.array(next_static_adj), dtype=T.float).to(device)
        next_dynamic_adj_tensor = T.tensor(np.array(next_dynamic_adj), dtype=T.float).to(device)
        next_insert_adj_tensor = T.tensor(np.array(next_insert_adj), dtype=T.float).to(device)
        next_blank_node_tensor = T.tensor(np.array(next_blank_node), dtype=T.float).to(device)
        next_insert_node_tensor = T.tensor(np.array(next_insert_node), dtype=T.float).to(device)
        ...
        with T.no_grad():
            V_, A_, _, _ = self.q_target.forward(next_node_fea_tensor,
                                                 next_static_adj_tensor,
                                                 next_dynamic_adj_tensor,
                                                 next_insert_adj_tensor,
                                                 next_blank_node_tensor,
                                                 next_insert_node_tensor,
                                                 hn_tensor, cn_tensor)
        ...
        V, A, _, _ = self.q_eval.forward(node_fea_tensor,
                                         static_adj_tensor,
                                         dynamic_adj_tensor,
                                         insert_adj_tensor,
                                         blank_node_tensor,
