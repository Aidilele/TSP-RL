...
class DuelingNetWork(nn.Module):
    ...
    def forward(self, node_fea, node_adj, last_node, h, c):
...
class DuelingDQN:
    ...
    def choose_action(self, state, h, c, isTrain=True):
        ...
        if (np.random.random() < self.epsilon) and isTrain:
            ...
            last_node = T.tensor(state[5], dtype=T.int32).to(device)
            h = T.tensor(h, dtype=T.float).to(device)
            c = T.tensor(c, dtype=T.float).to(device)
            _, A, hn, cn = self.q_eval.forward(node_fea, node_adj, last_node, h, c)
        else:
            node_fea = T.tensor(state[0], dtype=T.float).to(device)
            node_adj = T.tensor(state[1], dtype=T.int32).to(device)
    ...
    def learn(self):
        ...
        [node_fea, node_adj, action_mask,last_node,
         h, c, action, reward,
         next_node_fea, next_node_adj,
         next_action_mask,next_last_node,
        ...
        last_node_tensor = T.tensor(np.array(last_node), dtype=T.float).to(device)
        ...
        next_last_node_tensor = T.tensor(np.array(next_last_node), dtype=T.float).to(device)
