...
def main():
    ...
    state_dim = gat_output_dim*2
    action_dim = gat_output_dim
    ...
    ep_ope_fea = np.zeros([ep_max_step, ope_num, ope_dim])
    ep_ope_adj = np.zeros([ep_max_step, ope_num, ope_num])

    ep_action_mask = np.zeros([ep_max_step, ope_num])
    ep_last_node = np.zeros([ep_max_step, ope_num])
    ...
    ep_next_ope_fea = np.zeros([ep_max_step, ope_num, ope_dim])
    ep_next_ope_adj = np.zeros([ep_max_step, ope_num, ope_num])

    ep_next_action_mask = np.zeros([ep_max_step, ope_num])
    ep_next_last_node = np.zeros([ep_max_step, ope_num])
    ...
    for episode in range(max_episodes):
        ...
        while not done:
            ...
            ep_action_mask[step] = state[2]
            ep_last_node[step] = state[5]
            ep_h[step] = h
            ep_c[step] = c
            ...
            ep_next_action_mask[step] = n_state[2]
            ep_next_last_node[step] = n_state[5]
            ep_hn[step] = hn
            ep_cn[step] = cn
            ep_done[step] = done
            ep_action[step] = action
            cmax_buffer.append(0.25-n_state[4])
            state = n_state
            h = hn
            c = cn
            step += 1
        ...
        agent.memory.store(ep_ope_fea[:ep_buffer_size],
                           ...
                           ep_next_last_node[:ep_buffer_size],
        ...
        reward_sum = sum(rewards)-0.25*len(rewards)
