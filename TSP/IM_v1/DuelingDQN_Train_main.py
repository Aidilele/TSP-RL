...
def main():
    ...
    improve_deepth = 10
    ep_node_fea = np.zeros([ep_max_step, ope_num, ope_dim])
    ep_static_adj = np.zeros([ep_max_step, ope_num, ope_num])
    ep_dynamic_adj = np.zeros([ep_max_step, ope_num, ope_num])
    ep_insert_adj = np.zeros([ep_max_step, improve_deepth + 1, improve_deepth + 2])
    ep_blank_node = np.zeros([ep_max_step, ope_num])
    ep_insert_node = np.zeros([ep_max_step, ope_num])
    ep_action_mask = np.zeros([ep_max_step, improve_deepth * (improve_deepth + 1)])

    ep_h = np.zeros([ep_max_step, state_dim])
    ep_c = np.zeros([ep_max_step, state_dim])
    ep_action = np.zeros([ep_max_step])
    ep_reward = np.zeros([ep_max_step])

    ep_next_node_fea = np.zeros([ep_max_step, ope_num, ope_dim])
    ep_next_static_adj = np.zeros([ep_max_step, ope_num, ope_num])
    ep_next_dynamic_adj = np.zeros([ep_max_step, ope_num, ope_num])
    ep_next_insert_adj = np.zeros([ep_max_step, improve_deepth + 1, improve_deepth + 2])
    ep_next_blank_node = np.zeros([ep_max_step, ope_num])
    ep_next_insert_node = np.zeros([ep_max_step, ope_num])
    ep_next_action_mask = np.zeros([ep_max_step, improve_deepth * (improve_deepth + 1)])

    ep_hn = np.zeros([ep_max_step, state_dim])
    ep_cn = np.zeros([ep_max_step, state_dim])
    ...
    for episode in range(max_episodes):
        ...
        step = 0
        while not done:
            state = env.graph.get_improve_fea()
            ep_node_fea[step] = state[0]
            ep_static_adj[step] = state[1]
            ep_dynamic_adj[step] = state[2]
            ep_insert_adj[step] = state[3]
            ep_blank_node[step] = state[4]
            ep_insert_node[step] = state[5]
            ep_action_mask[step] = state[6]
            ...
            ep_next_node_fea[step] = n_state[0]
            ep_next_static_adj[step] = n_state[1]
            ep_next_dynamic_adj[step] = n_state[2]
            ep_next_insert_adj[step] = n_state[3]
            ep_next_blank_node[step] = n_state[4]
            ep_next_insert_node[step] = n_state[5]
            ep_next_action_mask[step] = n_state[6]

            ep_hn[step] = hn
            ep_cn[step] = cn
            ep_done[step] = done
            ep_action[step] = action
            cmax_buffer.append(reward)
            state = n_state
            h = hn
            c = cn
            step += 1
            if not done:
                env.graph.generate(env.graph.generate_action())

        # rewards = env.reward(reward_type, cmax_buffer, reward_info)
        rewards = cmax_buffer
        ep_buffer_size = len(rewards)
        ep_reward[:ep_buffer_size] = rewards
        agent.memory.store(ep_node_fea[:ep_buffer_size],
                           ep_static_adj[:ep_buffer_size],
                           ep_dynamic_adj[:ep_buffer_size],
                           ep_insert_adj[:ep_buffer_size],
                           ep_blank_node[:ep_buffer_size],
                           ep_insert_node[:ep_buffer_size],
                           ...
                           ep_next_node_fea[:ep_buffer_size],
                           ep_next_static_adj[:ep_buffer_size],
                           ep_next_dynamic_adj[:ep_buffer_size],
                           ep_next_insert_adj[:ep_buffer_size],
                           ep_next_blank_node[:ep_buffer_size],
                           ep_next_insert_node[:ep_buffer_size],
